...
try:
    import anthropic
except ImportError:  # Streamlit Cloud/로컬 설치 누락 시 graceful fallback
    anthropic = None
...
def get_anthropic_api_key() -> Optional[str]:
    """Read API key from Streamlit secrets first, then local environment."""
    try:
        secret_value = st.secrets.get("ANTHROPIC_API_KEY", None)
    except Exception:
        secret_value = None
    return secret_value or os.environ.get("ANTHROPIC_API_KEY")


def call_sonnet(question: str, context_summary: str, base_result: Optional[Dict[str, Any]] = None) -> str:
    """Call Claude Sonnet with the structured context produced by rag_engine.

    rag_engine.py is intentionally not modified. We first run the deterministic
    ontology/RAG/lookup pipeline with use_gemma=False, then ask Sonnet to
    explain only from that context.
    """
    if anthropic is None:
        return "Sonnet API 호출 실패: anthropic 패키지가 설치되어 있지 않습니다. requirements.txt를 확인해 주세요."

    api_key = get_anthropic_api_key()
    if not api_key:
        return "Sonnet API 호출 실패: ANTHROPIC_API_KEY가 설정되어 있지 않습니다. Streamlit secrets 또는 환경변수를 확인해 주세요."

    base_result = base_result or {}
    route = base_result.get("route", "-")
    structured_answer = base_result.get("answer", "")
    metadata = base_result.get("metadata", {}) or {}

    prompt = f"""
당신은 금융회사 내부 LP출자 검토 업무를 설명하는 시연용 AI입니다.
아래 제공된 구조화 컨텍스트에 근거해서만 답변하십시오.
컨텍스트에 없는 구체 회사명, 일자, 수익률, 내부 직원명, 시장 정보는 만들지 마십시오.
...
""".strip()
    ...
    try:
        ...
        for block in message.content:
            ...
            if text:
                parts.append(text)
        return "\n\n".join(parts).strip() or "Sonnet API가 빈 응답을 반환했습니다."
    except Exception as exc:
        return f"Sonnet API 호출 실패: {type(exc).__name__}: {exc}"
...
def run_left_system(question: str, graph: Any, chunks: List[Dict[str, Any]], alias: Dict[str, Any], lookup: Dict[str, Any], model_choice: str) -> Dict[str, Any]:
    """Run ontology+RAG answer path according to selected model.

    - Gemma: rag_engine directly calls local Ollama model.
    - Sonnet: rag_engine first creates deterministic context with LLM off,
      then this wrapper calls Anthropic Sonnet.
    - Structured: rag_engine only returns deterministic/template answer.
    """
    start = time.perf_counter()
    ...
    try:
        ...
        result = {
            "answer": f"시스템 응답 생성 중 오류가 발생했습니다: {type(exc).__name__}: {exc}",
            "route": "error",
            "context_summary": "rag_engine.answer_question 호출 실패",
        }
    ...
    if model_choice == "Sonnet 4.6 API":
        # Step 1 was deterministic context creation above with use_gemma=False.
        # Step 2 replaces only the natural-language answer with Sonnet output.
        result["answer"] = call_sonnet(question, result.get("context_summary", ""), result)
        result["model_used"] = "Sonnet 4.6 API"
    elif model_choice == "Gemma 4 e4b 로컬":
        result["model_used"] = "Gemma 4 e4b 로컬"
    else:
        result["model_used"] = "구조화 모드"
...
def render_model_and_kpi(model_choice: str) -> str:
    ...
    with c1:
        ...
        elif selected == "Sonnet 4.6 API" and not get_anthropic_api_key():
            st.info("Sonnet 4.6 API 선택됨: ANTHROPIC_API_KEY가 없으면 호출 실패 안내가 표시됩니다.")
...
<div class="answer-card ontology-card">
  <div class="card-head">
    ...
    <div class="pill traceable">TRACEABLE</div>
        ...
        meta = "질문 대기"
        if left_result:
            meta = f"{left_result.get('elapsed', 0):.2f}s · {left_result.get('route', '-')} · {left_result.get('model_used', '-')}"
        ...
        if left_result:
            ...
            render_html("<p class='answer-placeholder'>질문을 실행하면 좌측에 온톨로지+RAG 답변이 표시됩니다.</p>")
        render_html("</div>")
  ...
  <div class="card-head">
    ...
    <div class="pill baseline">BASELINE</div>
        ...
        if right_result:
            ...
            render_html("<p class='answer-placeholder'>질문을 실행하면 우측에 RAG only 비교군 답변이 표시됩니다.</p>")
        render_html("</div>")
